...
public class CategoryController {
    ...
    private final CategoryPrint catPrint;

    public CategoryController() {
        catService = new CategoryService();
        catPrint = new CategoryPrint();
    }
    ...
    public void selectCategoryList(Map<String, String> parameter) {
        if(parameter.containsValue("orderList")) {
            List<CategoryDTO> catList = catService.selectCategoryList(parameter);

            if (catList != null) {
                catPrint.printTeamList(catList);
            } else {
                catPrint.printErrorMessage("selectList");
            }
        } else {
            List<CategoryDTO> catList = catService.selectCategoryList(parameter);
            ...
            if (catList != null) {
                ...
                catPrint.printErrorMessage("selectAllList");
            }
        }
    }
        // 2. 제품분류 목록을 조회하는 메소드
    ...
    public void registNewCategory(CategoryDTO catDTO) {
        boolean result = catService.registNewCategory(catDTO);


       if(result) {
           catPrint.printSuccessMessage("insert");
       } else {
           catPrint.printErrorMessage("insert");
       }
        // 3. 제품분류 정보를 등록하는 메소드
    ...
    public void modifyCategoryName(CategoryDTO catDTO) {
        boolean result = catService.modifyCategoryName(catDTO);

        if(catService.modifyCategoryName(catDTO)) {
            catPrint.printSuccessMessage("update");
        } else{
            catPrint.printErrorMessage("update");
        }
        // 4. 제품분류 정보를 수정하는 메소드
    ...
    public void deleteCategory(Map<String, String> parameter) {
        boolean result = catService.deleteCategory(parameter);

        if(result) {
            catPrint.printSuccessMessage("delete");
        } else {
            catPrint.printErrorMessage("delete");
        }
